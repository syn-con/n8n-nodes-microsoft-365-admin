...
const { executeLicenseWrite } = await import('./licenseWrite');
		...
		async (_credentialType: string, options: IHttpRequestOptions) => {
			...
			if (options.url.endsWith('/subscribedSkus')) {
				...
				return {
					statusCode: 200,
					headers: {},
					body: scenario.subscribedSkus ?? { value: [] },
				};
			}
			...
			// The transport always asks for the full response so it can read the status line
			// itself; this stub answers the same way n8n's helper would.

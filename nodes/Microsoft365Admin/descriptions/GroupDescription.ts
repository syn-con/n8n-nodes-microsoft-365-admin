...
// control-character half of that range is part of the rule Graph enforces, so the
// no-control-regex warning is expected here rather than accidental.
// eslint-disable-next-line no-control-regex
const ASCII_MAIL_NICKNAME = /^((?![@()[\]"\\;:<> ,])[\x00-\x7F])*$/;
		...
		options: [
			{
				...
				routing: {
					request: {
						...
						method: 'POST',
						url: '=/groups/{{ $parameter["group"] }}/owners/$ref',
					},
					output: {
						postReceive: [
			...
			},
			{
				...
				routing: {
					request: {
						ignoreHttpStatusErrors: ignoreHttpStatusErrorsConfig,
						method: 'GET',
						url: '=/groups/{{ $parameter["group"] }}/members',
					},
					output: {
						postReceive: [
							handleErrorPostReceive,
			...
			},
			{
				...
				routing: {
					request: {
						ignoreHttpStatusErrors: ignoreHttpStatusErrorsConfig,
						method: 'GET',
						url: '=/groups/{{ $parameter["group"] }}/owners',
					},
					output: {
						postReceive: [
							handleErrorPostReceive,
			...
			},
			{
				...
				routing: {
					request: {
						ignoreHttpStatusErrors: ignoreHttpStatusErrorsConfig,
						method: 'DELETE',
						url: '=/groups/{{ $parameter["group"] }}/owners/{{ $parameter["user"] }}/$ref',
					},
					output: {
						postReceive: [
							handleErrorPostReceive,
...
/** The group picker, reused by every operation that targets a single group. */
const groupLocator = (operation: string, description: string): INodeProperties => ({
	displayName: 'Group',
	name: 'group',
	default: {
		mode: 'list',
...
/** Return All / Limit pair for the collection endpoints that support `@odata.nextLink`. */
const paginationFields = (operation: string): INodeProperties[] => [
	{
		displayName: 'Return All',
		name: 'returnAll',
		default: false,
...
const addOwnerFields: INodeProperties[] = [
	groupLocator('addOwner', 'The group to add the owner to'),
	{
		displayName: 'User to Add as Owner',
		name: 'user',
		default: {
			mode: 'list',
			value: '',
...
const getMembersFields: INodeProperties[] = [
	groupLocator('getMembers', 'The group whose members should be retrieved'),
	...paginationFields('getMembers'),
];

const getOwnersFields: INodeProperties[] = [
	groupLocator('getOwners', 'The group whose owners should be retrieved'),
	...paginationFields('getOwners'),
];

const removeOwnerFields: INodeProperties[] = [
	groupLocator('removeOwner', 'The group to remove the owner from'),
	{
		displayName: 'Owner to Remove',
		name: 'user',
		default: {
			mode: 'list',
			value: '',

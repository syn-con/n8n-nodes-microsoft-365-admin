...
/**
 * A target is an object ID or a userPrincipalName, neither of which contains any of these.
 * They are rejected rather than escaped because a `?` or `/` arriving from workflow data
 * would otherwise reshape the request URL — `alice/revokeSignInSessions?x=` would turn the
 * intended `POST /users/alice/assignLicense` into a POST to a different Graph endpoint.
 */
...
async function buildWrite(
	...
	if (UNSAFE_IN_TARGET.test(target)) {
		throw new NodeOperationError(
			this.getNode(),

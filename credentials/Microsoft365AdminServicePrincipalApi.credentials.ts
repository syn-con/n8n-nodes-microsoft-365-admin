...
import { buildClientAssertion, CLIENT_ASSERTION_TYPE } from './common/client-assertion';
...
/**
 * Issues the token-exchange POST. Community packages cannot reach n8n's internal
 * outbound-HTTP container, so the caller passes in the `httpRequest` helper that
 * `preAuthentication` receives on `this`.
 */
type TokenPoster = (options: IHttpRequestOptions) => Promise<unknown>;
...
export async function getAccessToken(
	...
	if (
		normalizedBaseUrl &&
		!Object.prototype.hasOwnProperty.call(LOGIN_HOSTS_BY_GRAPH_URL, normalizedBaseUrl)
	) {
	...
	// so the origin is not user-controlled.
...
export class Microsoft365AdminServicePrincipalApi implements ICredentialType {
	name = 'microsoft365AdminServicePrincipalApi';

	displayName = 'Microsoft 365 Admin (Service Principal) API';
